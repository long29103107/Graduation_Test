...
class CheckoutController extends Controller
{
    ...
    //     Session::put('check','login_checkout');
    //     return view('auth.login');
    // }
    // public function order_place(Request $request){
      
    //     //Không validator request
    //     $payment_data = array();
    //     $payment_data['method'] = $request->payment_select;
    //     $payment_data['status'] = 1;
    //     $payment_data['amount'] = (int) Cart::subtotal();
    //     $payment_data['fee'] = 0;
    //     $payment_id = DB::table('payments')->insertGetId($payment_data);//Lấy id của payment vừa insert
    //     //Thêm chi tiết đơn đặt Hoàng
    //     $order = new Order();
    //     $order->payment_id = $payment_id;
    //     $order->total = (int) Cart::subtotal();
    //     $order->fullname = $request->shipping_name;
    //     $order->email = $request->shipping_email;
    //     $order->phone = $request->shipping_phone;
    //     $order->address = $request->shipping_address;
    //     $order->save();

    //     //save như vầy nè mới có created_ad với updated_at
    //     //cho hỏi chô này
    //     //là order hay other
    //     //save lại tự nó có id mà dùng thôi 

    //     // $order_data = array();
    //     // $order_data['payment_id'] = $payment_id;
    //     // $order_data['total'] = (int) Cart::subtotal();
    //     // $order_data['fullname'] = $request->shipping_name;
    //     // $order_data['email'] = $request->shipping_email;
    //     // $order_data['phone'] = $request->shipping_phone;
    //     // $order_data['address'] = $request->shipping_address;
    //     // $order_id = DB::table('orders')->insertGetId($order_data);//Lấy id của payment vừa insert

    //     //chỗ này đặt hàng được chưa ?
    //     //đăt đc r, luu lên DB chứ chưa gửi qua mail đc
    //     //mở database lên xem
    //     //thử 
    //     //save datatbase dùng model luôn 
    //     //Không insert

    //     return view('user.order_place');
    // }
    // public function order_place_ajax(Request $request){
    ...
    //     return view('user.order_place_ajax');
    // }

    public function checkout(){
        // session()->forget('cart');
        // session()->forget('discount_cart');
        // dd(session()->get('cart'),session()->get('discount_cart'));
    ...
    // public function paymentVNPay($request)
    // {
    //     $session_order =Session::get('order');

    //     $order = new Order();  
    //     $order->fullname = $session_order['fullname'];
    ...
    //     $cart = json_decode($session_order['list_product']);
    //     $total_sale = 0;
    //     if(!empty($session_order['discount']))
    //     {
    //         $discount = Discount::find($session_order['discount']);
    //         if(!empty($discount))
    //         {
    //             $list_id_apply = json_decode($discount->apply);
    ...
    //     $payment->method = "online";
    //     $payment->order_id = $order->id;
    //     $payment->ship_id = getLimitFee($total)->id;
    //     $payment->payment_gateway = $session_order['payment'];
    //     $payment->code_bank =  $session_order['bank'];
    //     $payment->payment_log =  json_encode($request->all());
    //     $payment->time = Carbon::parse($request->vnp_PayDate)->format('Y-m-d H:i:s');
    //     if($payment->method == "online")
    //     {
    //         $payment->status = 1;
    //     }
    //     $payment->amount = $total + getLimitFee($total)->fee;
    //     $payment->save();
